...
transformed parameters {
    ...
    
	{
		...
		for (i in 1:N) {
            ...
            omega[i] = fmax(0.001, fmin(0.999, inv_cloglog(pi_log[XQ[i],1] + n[XQ[i],1] * biomass_log)));
            
            // conditional expected catch
            mu_log[i] = pi_log[XQ[i],2] + n[XQ[i],2] * biomass_log - log(omega[i]) - pow(sigma[XQ[i], XY[i]], 2) / 2;
...
generated quantities {
	...
	real encounter_rate[Q]     = exp(pi_log[1:Q,1]);
    ...
    // CATCH PREDICTION
    {
        ...
        for (i in 1:N) {
            ...
            catch_hat_predict[XQ[i], XG[i]] += catchability[XQ[i]] * pow(biomass_hat, n[XQ[i],2]);
        }
    }
	...
	catchability_trace[1] = vector_norm(to_vector(pi_log[1:Q,1]));
    catchability_trace[2] = vector_norm(to_vector(pi_log[1:Q,2]));
    catchability_trace[3] = vector_norm(to_vector(n[1:Q,1]));
    catchability_trace[4] = vector_norm(to_vector(n[1:Q,2]));
