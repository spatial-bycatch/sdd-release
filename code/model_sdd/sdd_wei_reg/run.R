...
if (file.exists(file.path(run_path, 'inits.rda'))) {
	...
	mdl.tmp <- optimizing(mdl, mdl.dat, init = mdl.ini, verbose = TRUE, as_vector = FALSE, iter = 10)
} else { 
...
dfr <- posterior(mdl.fit, pars = "pi_log", dim.names = list(list(iter = 1:nit, group = c("Survey", "Prior"))), melt = TRUE)[[1]]

gg <- ggplot(dfr) + 
    geom_histogram(aes(x = value, y = after_stat(density)), position = "identity", binwidth = 0.5) + 
    labs(x = "", y = "") + theme_bw(base_size = 20) + facet_grid(group~"Efficiency") + theme(axis.text.y = element_blank())
ggsave(gg, file = file.path(res_path, "posteriors.png"), width = 12)
