...
transformed parameters {
	...
	
	{
		...
		for (i in 1:N) {
            ...
            omega[i] = inv_cloglog(pi_log[1] + biomass_log);
            
            // conditional expected catch
            mu_log[i] = pi_log[1] + biomass_log - log(omega[i]) - pow(sigma[XY[i]], 2) / 2;
...
generated quantities {
	...
	error_trace[1] = vector_norm(sigma);
	error_trace[2] = tau;
	error_trace[3] = rho;
}
