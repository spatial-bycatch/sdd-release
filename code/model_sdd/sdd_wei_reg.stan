...
functions {
	...
	*/
	real log_beta_lpdf(real x, real a, real b) {
		...
		real lp = a * x + (b - 1) * log1m_exp(x) + lgamma(a + b) - lgamma(a) - lgamma(b);
...
transformed parameters {
	...
	
	{
		...
		for (i in 1:N) {
            ...
            omega[i] = inv_cloglog(pi_log[1] + n[1] * biomass_log);
            
            // conditional expected catch
            mu_log[i] = pi_log[1] + n[1] * biomass_log - log(omega[i]) - pow(sigma[XY[i]], 2) / 2;
...
generated quantities {
   ...
   // expected and simulated 
	...
	real prior_catchability     = exp(pi_log[2]);
	real prior_catchability_phi = n[2];
    ...
    // CATCH PREDICTION
    {
        ...
        for (i in 1:N) {
            ...
            catch_hat_predict[XG[i]] += catchability * pow(biomass_hat, n[1]);
        }
    }
	...
	catchability_trace[1] = pi_log[1];
    catchability_trace[2] = n[1];
