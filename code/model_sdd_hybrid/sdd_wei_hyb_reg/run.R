...
save(dat_path, run_path, res_path, fig_path, stn_path, mdl_name, mdl_spp, file = "directories.rda")
...
if (file.exists(file.path(run_path, 'inits.rda'))) {
	...
	mdl.tmp <- optimizing(mdl, mdl.dat, init = mdl.ini, verbose = TRUE, as_vector = FALSE)
} else { 
...
mdl.ini$n           <- mdl.tmp$par$n
mdl.ini$reg_par     <- mdl.tmp$par$reg_par
mdl.ini$sigma       <- mdl.tmp$par$sigma

save(mdl.ini, file = file.path(run_path, 'inits.rda'))

# save initial pi values
dfr <- data.frame(group = c(X.dimnames$group, "PRIOR"), pi_log = round(mdl.ini$pi_log, 2), pi = round(exp(mdl.ini$pi_log), 2))
write.csv(dfr, file = "pi_init.csv", row.names = FALSE)

# MCMC run
mdl.fit <- sampling(mdl, mdl.dat, init = function() mdl.ini, iter = n_iter, cores = n_cores, chains = n_chains, thin = n_thin)
...
dfr <- posterior(mdl.fit, pars = "pi_log", dim.names = list(list(iter = 1:nit, group = c("Survey", "Prior"), catchability = c("Encounter Rate", "Efficiency"))), melt = TRUE)[[1]]

gg <- ggplot(dfr) + 
    geom_histogram(aes(x = value, y = after_stat(density)), position = "identity", binwidth = 0.5) + 
    labs(x = "", y = "") + theme_bw(base_size = 20) + facet_grid(group~catchability) + theme(axis.text.y = element_blank())
ggsave(gg, file = file.path(res_path, "posteriors.png"), width = 14)
