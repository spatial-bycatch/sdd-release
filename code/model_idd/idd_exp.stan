...
transformed parameters {
    ...
    
	{
		...
		for (i in 1:N) {
            ...
            omega[i] = fmax(0.001, fmin(0.999, inv_cloglog(pi_log[XQ[i]] + biomass_log)));
            
            // conditional expected catch
            mu_log[i] = pi_log[XQ[i]] + biomass_log - log(omega[i]) - pow(sigma[XQ[i], XY[i]], 2) / 2;
