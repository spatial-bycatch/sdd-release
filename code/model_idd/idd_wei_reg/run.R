...
mdl.ini <- posterior(mdl.fit, pars = c("density_log", "pi_log", "n", "reg_par", "sigma", "rho", "tau"), fun = "median")
save(mdl.ini, file = file.path(run_path, "inits.rda"))
...
dfr <- posterior(mdl.fit, pars = "pi_log", dim.names = list(list(iter = 1:nit, group = c(X.dimnames$group, "Prior"))), melt = TRUE)[[1]]
